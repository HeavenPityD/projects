...
void heap<T, Compare>::heapifyDown(size_t currentIdx)
{
    ...
    size_t max_Idx = maxPriorityChild(currentIdx);
    if (higherPriority(_elems[max_Idx], _elems[currentIdx])) {
        std::swap(_elems[currentIdx], _elems[max_Idx]);
        heapifyDown(max_Idx);
    }
...
heap<T, Compare>::heap(const std::vector<T>& elems) 
{
    ...
    if (elems.size() != 0) {
        ...
        size_t cur_idx = _elems.size() - 1;
        while(!hasAChild(cur_idx)) {
            cur_idx--;
        }
        while (cur_idx >= 1) {
            heapifyDown(cur_idx);
